...
const FullInventoryDash = ({
	...
	const extractionBox = useMemo(() => {
		...
		const eachAndQuantity = inventoryManagement.map((box) => {
			return { name: box.Product, eachAmount: box.quantityS };
		});
		...
		(function () {
			const merging = eachAndQuantity.map((box) => {
				if (merged.some((obj: MerginProps) => obj.name === box.name)) {
					...
					const key = merged.findIndex(
						(key) => JSON.stringify(key.name) === JSON.stringify(box.name)
					);
					if (key !== -1) {
						// console.log(key);
