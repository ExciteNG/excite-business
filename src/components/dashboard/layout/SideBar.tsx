...
const SideBar = () => {
	...
	const userType = getCookie("eb_usr");

	const sideTabs = useCustomSideBar({
		userType: userType === "BUSINESS" ? "SUPERAGENT" : "MAINAGENT",
	});
	...
	return (
		<div className="fixed left-0 top-0 h-screen w-[17rem] bg-[#101828]">
			<div className="w-full p-8 " onClick={() => router.push("/")}>
				<Image
					src={exciteBusinessLogo}
					alt="exciteBusiness logo"
						...
						<div key={index} className="shadow">
							{tab.tab === "Agents" ? (
								...
								<Link href={tab.tabLink}>
									<div
										className={`${
											pathName === tab.tabLink && "text-[#A7CC48] font-semibold"
